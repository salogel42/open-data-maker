		...
		describe "with one term" do
          ...
          describe "exporting csv" do
			...
			it "handles a csv format request" do
			  expect(last_response).to be_ok
				expect(last_response.content_type).to eq('text/csv')

				result = CSV.parse(last_response.body)
				
				expect(result.length).to eq 2
				
				expect(result[0]).to eq ["state", "name", "population", "land_area", "location.lat", "location.lon"]
				expect(result[1]).to eq ['IL', 'Chicago', 2695598, 227.635, 41.837551, -87.681844]
			end
		end
